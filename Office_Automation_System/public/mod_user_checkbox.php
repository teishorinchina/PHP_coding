...
function dep_data_user($fid=0,$userview){
	global $db;
	$query = $db->query("SELECT * FROM ".DB_TABLEPRE."user a,".DB_TABLEPRE."user_view b WHERE a.id=b.uid and a.departmentid  LIKE '%,".$fid.",%' order by a.numbers asc");
	$html='';
	while ($row = $db->fetch_array($query)) {
		
		$html.= 'subarr'.$fid.'.push( {'.chr(13).chr(10); 
		$html.= ' "id" : "2'.$fid.$row['id'].'",'.chr(13).chr(10);
		$html.= '  "text" : "'.$row['name'].'",'.chr(13).chr(10);
