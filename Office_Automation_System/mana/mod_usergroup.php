...
if ($do == 'list') {
    ...
    get_key("config_usergroup_Increase");
	$_title['name']='发布';
	...
	if ($savetype == 'edit') {
		...
		$sql = "SELECT id FROM ".DB_TABLEPRE."user where groupid='".$id."'";
		$query = $db->query($sql);
		while ($row = $db->fetch_array($query)) {
			$db->query("update ".DB_TABLEPRE."user_view set homemana='' WHERE uid = ".$row['id']." ");
		}
		usergroup_recache();
		if($id!=''){
