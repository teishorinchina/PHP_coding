...
class User {
				...
				if($user[loginip]!=''){
					$sqlip = "SELECT * FROM ".DB_TABLEPRE."user WHERE id=".$user[id]." and loginip like'%".get_onlineip()."%'";
						if ( $userip = $this->db->fetch_one_array($sqlip) ) {
								...
								$this->db->update('user',array('online' => 1), array('id' => $user['id']));
								return 1;
							}else{
							return -5;
							}
	...
	function logout() {
		...
		$this->db->update('online',$updateonline, array('uid' => $this->id));
		$updateuser = array(
			'online' => 0
		);
		$this->db->update('user',$updateuser, array('id' => $this->id));
		$this->id = 0;
		set_cookie('auth');
	}
	...
	function add_session($data) {
		...
		$this->db->insert('session', trim($data), true);
	}
}
?>