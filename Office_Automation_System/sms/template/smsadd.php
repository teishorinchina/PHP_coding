...
<table width="70%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
	...
	<a href="admin.php?ac=smsindex&fileurl=<?php echo $fileurl?>" style="font-size:12px;"><<返回列表页</a></span>
    </td>
  </tr>
</table>
...
<table class="TableBlock" border="0" width="70%" align="center" style="border-bottom:#4686c6 solid 0px;">
    <tr>
	  ...
	  <input type="hidden" name="receiveid" value="" />
        <textarea name="receive" cols="50" rows="5" style="width:400px; height:80px;" class="disabledInput"><?php if($_GET["name"]==''){?>请选择人员<?php }else{ echo $_GET["name"]; }?>
        </textarea>
        <br><a href="#" onClick="window.open ('admin.php?ac=user_checkbox&fileurl=public&inputname=receive', 'newwindow', 'height=500, width=500, top=50, left=100, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no')" class="FuncBtn">+选 择</a> 
	  </td>
    </tr>
    <tr>
      <td nowrap class="TableContent"> 接收人手机号码：<? get_helps()?></td>
      <td class="TableData">
        <textarea name="receivephone" cols="60"  style="width:400px; height:80px;" rows="4" class="disabledInput"><?php echo $_GET["phone"]?></textarea>
	  <br>注：多个手机号码请用英文","号隔开
      </td>
    </tr>
    <tr>
      <td nowrap class="TableContent">内容：<? get_helps()?></td>
      <td class="TableData"><textarea name="content" cols="70" rows="7" class="InputAreaStyle" id="youbian" onkeyup="checkLength(this);"></textarea>
      <br>
	  最长可输入68个汉字，您还可以输入
