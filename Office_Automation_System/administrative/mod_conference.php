...
if($ischeck=getGP('ischeck','G')){
	$_check['ischeck'.getGP('ischeck','G').'']='  class="active"';
}else{
	$_check['ischeck']='  class="active"';
}
...
if ($do == 'list') {
	...
	$un = getGP('un','G');
	$ui = getGP('ui','G');
	if(!is_superadmin() && $ischeck=='' && $ui==''){
		$wheresql .= " and (uid='".$_USER->id."' or staffid='".$_USER->id."')";
	}
	if ($ui!='') {
	...
	if($_POST['view']!=''){
		...
		if($id!=''){
			...
			if(getGP('sms_info_box_participation','P')!=''){
				$content='';
				$content='您有一个会议需要出席,申请人是:"'.$apppersonsms.'",请注意查看会议出席时间!';
			...
			if(getGP('sms_info_box_staffid','P')!=''){
				$content='';
				$content=$staffidsms.':有一个会议申请需要您审批,申请人是:"'.$apppersonsms.'",请进入会议管理进行审批!';
