...
if ($do == 'list') {
	...
	if($_GET['view']=='add'){
		$id = getGP('menuid','G');
		$idarr=explode('_',$id);
		$homemana=explode('|',$bghome['homemana']);
		$html='';
		for($i=0;$i<sizeof($homemana);$i++){
			$j=$j+1;
			if($idarr[0]==$j){
				$html.=$homemana[$i].'"'.$idarr[1].'",|';
			}else{
				$html.=$homemana[$i].'|';
			}
		}
	...
	if($_GET['view']=='removeIcon'){
		$id = getGP('menuid','G');
		$user_view = array(
			'homemana' => str_replace('"'.trim($id).'",','',$bghome['homemana'])
		);
		update_db('user_view',$user_view, array('uid'=>$_USER->id));
	}
	if($_GET['view']=='moveIconTo'){
		$id = getGP('menuid','G');
		$idarr=explode('_',$id);
		$homemana=str_replace('"'.$idarr[1].'",','',$bghome['homemana']);
