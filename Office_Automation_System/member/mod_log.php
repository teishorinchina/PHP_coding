...
if ($do == 'list') {
	...
	//生成数据
		...
		$sql = "SELECT * FROM ".DB_TABLEPRE."loginlog WHERE 1 $wheresql and uid='".$_USER->id."'  ORDER BY id desc";
		$result = $db->query($sql);
	...
	$num = $db->result("SELECT COUNT(*) AS num FROM ".DB_TABLEPRE."oalog WHERE 1 $wheresql and uid='".$_USER->id."'");
     $sql = "SELECT * FROM ".DB_TABLEPRE."oalog WHERE 1 $wheresql and uid='".$_USER->id."' ORDER BY id desc LIMIT $offset, $pagesize";
	 $result = $db->fetch_all($sql);
